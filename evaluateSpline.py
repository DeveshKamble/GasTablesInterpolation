def evaluateSpline(x, splines):
    for spline in splines:
        if spline['Mach_start'] <= x <= spline['Mach_end']:
            ...
            result = round(spline['a'] + spline['b']*dx + spline['c']*pow(dx,2) + spline['d']*pow(dx,3),3)
            return result
...
def evaluateSplineDerivative(x, splines):
    if x < splines[0]['Mach_start']:
        ...
        spline = splines[0]
        dx = x - spline['Mach_start']
        return round(spline['a'] + spline['b']*dx + spline['c']*pow(dx,2) + spline['d']*pow(dx,3),3)
    for spline in splines:
        if spline['Mach_start'] <= x <= spline['Mach_end']:
            dx = x - spline['Mach_start']
            result = round(spline['a'] + spline['b']*dx + spline['c']*pow(dx,2) + spline['d']*pow(dx,3),3)
    ...
    spline = splines[-1]
    dx = x - spline['Mach_start']
    return round(spline['b'] + 2*spline['c']*pow(dx,1) + 3*spline['d']*pow(dx,2),4)
